...
*/
// I accidentally did this one using filter rather than forEach but it works. Whoops!
function onlyEvenValues(arr){
    let evenValues=  arr.filter(function(value){
        return value%2 == 0;
    })
    return evenValues;
}
...
function vowelCount(str) {
    ...
    splitString.forEach(function(letter) {
      ...
      if (vowels.indexOf(lowerCasedLetter) !== -1) {
        if (vowelCount[lowerCasedLetter]) {
          vowelCount[lowerCasedLetter]++;
        } else {
          vowelCount[lowerCasedLetter] = 1;
        }
      }
    });
    return vowelCount;
...
/*
Write a function called find which accepts an array and a value and returns the first element in the array that has the same value as the 
